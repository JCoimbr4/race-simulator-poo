...
void Consola::drawRectangle(int l, int c, int x, int y) {
	int i;
	Consola::gotoxy(x, y);
	std::cout << char(218);
	for (i = 0; i < l - 2; i++)
...
//vai para uma posicao e escreve string
void Consola::goAndWrite(int x, int y, const std::string s) {
	COORD coord;
	coord.X = x;
	coord.Y = y;
	SetConsoleCursorPosition(hconsola, coord);
...
int modeInterface() {
	...
	Consola::goAndWrite(2, 1, "--Programacao Orientada a Objectos 2019/2020--");
	Consola::goAndWrite(2, 2, "--Trabalho Pratico--");
	Consola::goAndWrite(2, 3, "--Simulador de Corridas de Carros--");

	Consola::drawRectangle(50, 7, 0, 5);

	Consola::goAndWrite(2, 6, "Mode 1");
	Consola::goAndWrite(2, 8, "Mode 2");
	Consola::goAndWrite(2, 10, "Exit");


	char tecla;
	int x = 1, y = 6;
	Consola::goAndWrite(x, y, "*");
	while (1) {
		tecla = Consola::getch();
		if (tecla == Consola::ENTER) {
			if (y == 6)
				return 1;
			if (y == 8)
				return 2;
		...
		if ((tecla != Consola::CIMA) && (tecla != Consola::BAIXO)) continue;
